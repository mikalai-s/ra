...
function maxShiftsReachedForEmployee(eid, w, max, shiftRules) {
    let maxShiftsRule = shiftRules.filter(r => r.rule_id === MAX_SHIFTS_RULE_ID && r.employee_id === eid)[0];
    if (maxShiftsRule) {
        max = maxShiftsRule.value;
    }
...
function feature4(weekStart, weekEnd, employees, shiftRules, timeOffs) {
    const result = {};
    ...
    for (let w = weekStart; w <= weekEnd; w += 1) {
        ...
        for (let d = 1; d <= 7; d += 1) {
            ...
            while (true) {
                ...
                if (cw[d].indexOf(ce.id) === -1 && !maxShiftsReachedForEmployee(ce.id, cw, maxShiftsCorporate, shiftRules)) {
...
module.exports = function (weekStart, weekEnd) {
    return Promise.all(
        ...
        .then(([employees, shiftRules, timeOffs]) => feature4(weekStart, weekEnd, employees, shiftRules, timeOffs));
}